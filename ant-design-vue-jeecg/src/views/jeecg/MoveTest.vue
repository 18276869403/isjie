          ...
          <div>
            <span v-for="item in XYs"  v-bind="item.id">
                <a-modal
                  title="Title"
                  :visible="visible"
                  @ok="handleOk1(deleteid)"
                  @cancel="handleCancel1"
                >
                   <p>是否删除此图标</p>
                </a-modal>
                <a-row v-if="item.xposition!=null&&item.xposition!=''" style="position:absolute;">
                  <img :title="item.deviceNum" 
                       class="deviceImg" id="app" ref="tubiao" 
                       :src="src + item.picUrl" 
                       ...
                       :initBl="picbeilv"
                       :style="{top:(item.xyAxisDTO.yaxis*picbeilv+curY-40)+'px',
              ...
              <a-row v-if="item.xposition==null">
                <img :src="item.picUrdeviceNuml" @contextmenu.prevent="showModal1"  :id="item.xyAxisDTO.id" v-show="false"  :name="item.xyAxisDTO.id" @mousedown="PointMouseDown"
                     :style="{top:item.xyAxisDTO.yaxis*beilv+'px',left:item.xyAxisDTO.xaxis*beilv+'px',position:'absolute',width:'40px',height:'40px'}">
                <div v-show="false" :id="item.xyAxisDTO.id" :title="item.deviceNum" :style="{top:(item.xyAxisDTO.yaxis*beilv+40)+'px',left:(item.xyAxisDTO.xaxis*beilv-14)+'px',position:'absolute',color:'#000',background:'#FFEC8B',width: '20px'}" class="deviceNum">
                  {{ item.deviceNum }}
                </div>
              </a-row>
            </span>
...
<!--            <button onclick="" style="font-size: 15px">查找：{{123}}</button>-->
            ...
            <p style="font-size: 15px; margin-top: 10px ">楼层名：
              <select   style=" width: 133px;background-color: #4a4a48 "    @change='handleChange($event)'>
                <option style="background-color: #4a4a48"  v-for="(item,index) in options" :key="index.toString()"
                        v-if="item.id==paramF"  selected  :value="item.id">
                  {{item.floorName}}
                </option>
                <option style="background-color: #4a4a48"  v-for="(item,index) in options" :key="index.toString()"
                        v-if="item.id!=paramF" :value="item.id">
                  {{item.floorName}}
                </option>
              </select>
              <a-button type="primary" @click="saveButton" style="margin-left: 20px;height: 35px;" >保存</a-button>
            </p>
            ...
            <a-row>
              ...
              <a-col span="4">
                <a-button type="primary" @click="searchBydeviceNum()" style="margin-left: 10px height: 35px;" >查找</a-button>
              </a-col>
              <a-col span="7"></a-col>
          ...
          <div style="height:600px;margin-left: 20px;margin-right: 20px">
            <table style="background-color: #02172b;text-align: center;width:100%;border-color: white;table-layout: fixed"  border="1" >
              ...
              <tr  v-for="item in currentPageData" style="height: 40px">
                <td :style="{color:trColor(item)==null?'#FFF':trColor(item)}">{{item.deviceType}}</td>
                <td :style="{color:trColor(item)==null?'#FFF':trColor(item)}"
                    style="white-space: nowrap;
                           overflow: hidden;
                            text-overflow: ellipsis;"
                    :title="item.deviceNum">{{item.deviceNum}}</td>
                <td style= "color:#000000"><button type="primary"   @click="addOneDevice(item.id)" :disabled="toStatus(item)==null?null:toStatus(item)">添加</button>
          ...
          <footer style="margin-left: 25%;margin-top: 20px;">
            <button @click="prevPage()" style="color: #1890ff">
              上一页
            </button>
            <span style="margin-left: 10px;">第{{currentPage}}页/共{{totalPage}}页</span>
            <button @click="nextPage()" style="color: #1890ff;margin-left: 10px;">
              下一页
            </button>
          </footer>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getCurrentPageData() {
        ...
        this.currentPageData = this.datas.slice(
          begin,end
        );
      ...
      dragImg: function(e) { //拖动图片
        ...
        document.onmousemove = (e) => {//背景图片按下并移动的事件
          if (this.dragFlag) {
