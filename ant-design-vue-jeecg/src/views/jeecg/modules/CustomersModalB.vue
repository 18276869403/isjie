<template>
  <a-modal
    ...
    :footer="null"
    @cancel="handleCancel"
    style="height: 450px;position: absolute;top: 40%;left: 40%;margin-top: -230px;margin-left:-300px;">

    <a-spin :spinning="confirmLoading" style="height: 480px;">
      <a-form :form="form" style="margin-left: 32px;">
        ...
        <a-form-item
          ...
          <a-input placeholder="请选择客户名称" v-decorator="['name', {} ]" style="width: 100%;outline: none;height: 100%" readonly/>
        </a-form-item>
        ...
        <a-form-item
          ...
          <a-input placeholder="请输入项目名称" v-decorator="['projectName', {} ]" style="width: 100%;outline: none;height: 100%" readonly/>
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          label="经度" class="a-formItem">
          <a-input placeholder="请输入经度" v-decorator="[ 'longitude', validatorRules.longitude]" style="width: 100%;outline: none;height: 100%" readonly/>
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          label="纬度" class="a-formItem">
          <a-input placeholder="请输入纬度" v-decorator="[ 'latitude', validatorRules.latitude] " style="width: 100%;outline: none;height: 100%" readonly/>
        </a-form-item>
        ...
        <template v-if="!model.id">
          <a-form-item
            ...
            <a-upload
              ...
              <img v-if="picUrl" :src="getAvatarView()" alt="客户logo" style="height:104px;max-width:104px;" />
              <div v-else>
        ...
        <template v-else>
          <a-form-item
            ...
            <a-upload
              ...
              @change="handleChange"
              v-decorator="['custLogo', {}]">
              <img v-if="picUrl" :src="getAvatarView()" alt="客户logo" style="left:20px;height:104px;max-width:104px;" />
              <div v-else>
                <a-icon :type="uploadLoading ? 'loading' : 'plus'" />
                <div class="ant-upload-text" >上传</div>
              </div>
        ...
        <!--鸟瞰图-->
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="鸟瞰图" class="a-formItem"  >
          <a-upload
            list-type="picture-card"
            class="avatar-uploader"
            :showUploadList="false"
            :action="uploadActionnk"
            :data="{'isup':1}"
            :headers="headersnk"
            :beforeUpload="beforeUploadnk"
            @change="handleChangenk" >
            <img v-if="nkpicUrl" :src="getAvatarViewnk()" alt="鸟瞰图" style="height:104px;max-width:104px;"  />
            <div v-else>
              <a-icon :type="uploadLoading ? 'loading' : 'plus'" />
              <div class="ant-upload-text" >上传</div>
            </div>
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="标注logo" class="a-formItem"  >
          <a-upload
            ...
            <img v-if="bzpicUrl" :src="getAvatarViewbz()" alt="标注logo" style="height:104px;max-width:104px;"  />
            <div v-else>
              <a-icon :type="uploadLoading ? 'loading' : 'plus'" />
              <div class="ant-upload-text" >上传</div>
            </div>
        ...
        <a-form-item
          ...
          <a-input placeholder="请输入客户详细地址" v-decorator="['detailedAddress', {}]"  style="width: 100%;outline: none;height: 100%" readonly/>
        </a-form-item>
        ...
        <a-form-item
          ...
          <a-input placeholder="请输入web端显示的名称" v-decorator="['webName', {}]" style="width: 100%;outline: none;height: 100%" readonly/>
        </a-form-item>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.headersnk = {"X-Access-Token":token}
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'oneAreaName','twoAreaName','name','projectName','detailedAddress','longitude','latitude','webName'))
          //时间格式化
        });
        this.picUrl = this.model.custLogo;
        this.nkpicUrl = this.model.backup1;
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let custLogo = that.model.custLogo;
            let backup1 = that.model.backup1;
            let biaoZhuLogo = that.model.biaoZhuLogo;

            let formData = Object.assign(this.model, values);
            formData.custLogo = custLogo;
            formData.backup1 = backup1;
            formData.biaoZhuLogo = biaoZhuLogo;
      ...
      handleChangenk (info) {
        ...
        if (info.file.status === 'done') {
          ...
          /*  console.log(response);*/
          if(response.success){
            this.model.backup1 = response.message;
            this.nkpicUrl = response.message;
          }else{
            this.$message.warning(response.message);
          }
        }
      ...
      getAvatarViewnk(){
        return this.url.imgerver +"/"+ this.nkpicUrl;
      },
