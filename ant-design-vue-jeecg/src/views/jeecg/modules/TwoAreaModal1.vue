      ...
      <a-form :form="form" style="height: 105px;margin-left: 25px;">
        ...
        <a-popconfirm v-if="this.$route.query.oneid!=null">
          <a-form-item label="选择省份" :labelCol="labelCol" :wrapperCol="wrapperCol" class="a-form-item">
            <a-select v-decorator="[ 'oneAreaId', validatorRules.oneAreaId]" placeholder="请选择省份" class="a-input" disabled>
              <a-select-option :value="oneAreaId"
                               @click="initialTwoAreaList(one.id)" >
                {{ oneAreaName }}
              </a-select-option>
            </a-select>
          </a-form-item>
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        validatorRules: {
          oneAreaId: { rules: [{ required: true, message: '请输入省份!'}] },
    ...
    methods: {
      initialTwoAreaList(id) {
        getAction("/monitor/oneArea/getoneall", { oneAreaId: id }).then((res) => {
      ...
      getOneArea(){
        if(this.$route.query.oneid!=null){
          var url = "/monitor/oneArea/queryById"
