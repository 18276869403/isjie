  ...
  <a-card :bordered="false">
          ...
          <a-col :md="4" :sm="8">
            <a-form-item label="城市">
              <a-select v-model="queryParam.twoAreaName" placeholder="请选择城市">
                ...
                <a-select-option v-for="(twoArea,twoAreaindex) in getTwoArea" :key="twoAreaindex.toString()"
                                 :value="twoArea.id">{{twoArea.areaName}}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :md="5" :sm="8">
    ...
    <div>
      ...
      <a-table ref="table"
        ...
        <!--鸟瞰图logo-->
        <span slot="logo3" slot-scope="text, record">
          <a v-if="record.backup1==null"></a>
          <a v-else @click="() => getUrlNewView(record.backup1)">{{text.slice(27)}}</a>
        </span>
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <router-link :to="{name: 'jeecg-DeviceAreaList', query: {customerId:record.id, oneAreaId: record.oneAreaId, twoAreaId: record.twoAreaId} }">分区</router-link>
         <a-divider type="vertical" />
          <router-link :to="{name: 'jeecg-DeviceAreaList', query: {customerId:record.id, oneAreaId: record.oneAreaId, twoAreaId: record.twoAreaId} }">标注</router-link>
        </span>
