...
public class JeecgController<T, S extends IService<T>> {
    ...
    protected void appendDeviceAreaAuth(HttpServletRequest req,
    		QueryWrapper<?> queryWrapper) {
		LoginUser sysUser = getLoginUser(req);
		queryWrapper.apply("("
    ...
    protected void appendCustomerAuth(HttpServletRequest req,
    		QueryWrapper<?> queryWrapper) {
		LoginUser sysUser = getLoginUser(req);
    	queryWrapper.apply("c.id in(select DISTINCT d.Customer_id from device d" + 
    			" where ifnull(d.device_area_id,'0') in(" + 
    			"	select '0' 	" + 
    			"	union  " + 
    			"	select dis.customer_id from distribution as dis" + 
    			"	where dis.user_id = {0}" + 
